...
class HDWeaponGrabber:HDWeapon{
	...
	action void A_CheckGrabbing(){
		...
		if(grabbed.bcorpse)grbng=grbng..StringTable.Localize("$FIST_CORPSE");
		else if(inventory(grabbed)||hdupk(grabbed))grbng=grbng..StringTable.Localize("$FIST_ITEM");
		else grbng=grbng..StringTable.Localize("$FIST_OBJECT");
...
class HDFist:HDWeaponGrabber replaces Fist{
	...
	action void HDPunch(double dmg){
		...
		//if no actor, check window busting and abort
		if(!punchee){
			HDF.Give(self,"WallChunkAmmo",1);
			if(punchline.hitline)doordestroyer.CheckDirtyWindowBreak(punchline.hitline,dmg*0.005,punchline.hitlocation);
			return;
		}
